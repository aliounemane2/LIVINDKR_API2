...
public class InscriptionController {
    ...
    public MessageResult handleFileUpload(@RequestPart("user") User user, @RequestPart(name="file",required=false) MultipartFile file,@RequestParam("type") Integer type) {
    ...
    private EmailStatus sendEmail(String code,String email, String template, Integer type){
    ...
    public HashMap<String, Object> ConfirmationEmail(@RequestParam("code") String token, @RequestParam( name="type",required= false) Integer type){
    	...
    	try {
            String token1 = type != null ? token : getToken(token);
        ...
        try {
            User getUserByEmail = userRepository.findByEmail(email);
                ...
                if(status.equals(1)){
                    ...
                    EmailStatus emailStatus = sendEmail(getUserByEmail.getActivationToken().toString(),getUserByEmail.getEmail(),TEMPLATE,0);
    ...
    public MessageResult updatePassword(@RequestParam("email") String email, @RequestParam("password") String password, @RequestParam("id") int id){
        if(id == 0){
            EmailStatus emailStatus = sendEmail(password,email,TEMPLATEPASSWORD,0);
            if(emailStatus.isError()){
